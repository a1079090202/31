  ...
  for (const r of rows) {
    lines.push(
      [r.order_no, r.frame_tail, r.brand, r.fault_desc,
        formatCents(r.labor_cents), formatCents(r.parts_cost_cents), formatCents(r.gross_cents), r.delivered_at]
        .map(esc).join(',')
    );
  }
  lines.push(
    ['合计', '', '', '', formatCents(totals.labor_cents), formatCents(totals.parts_cost_cents), formatCents(totals.gross_cents), '']
      .map(esc).join(',')
  );
  return '﻿' + lines.join('\r\n');
}
